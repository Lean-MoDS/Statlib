...
-/
import Statlib.LearningTheory.Rademacher.Massart
import Statlib.LearningTheory.Rademacher.Signs
import Statlib.Analysis.NormedSpace.CoveringNumber.L1
...
* `l1Norm`: compatibility view of Statlib's canonical `l1norm`.
* `L1Ball`: subtype associated with Statlib's canonical `l1Ball`.
...
-- Compatibility spelling for the canonical Statlib ℓ1 norm.
noncomputable abbrev l1Norm (w : EuclideanSpace ℝ (Fin d)) : ℝ :=
  l1norm w

-- Subtype of the canonical Statlib ℓ1 ball.
def L1Ball (W : ℝ) : Type :=
  ↥(l1Ball (d := d) W)
